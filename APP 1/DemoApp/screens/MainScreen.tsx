...
export default function MainScreen() {
  ...
  const [scanEnvironment, setScanEnvironment] = useState('emergency'); // Default to emergency environment for maximum reception
  ...
  // Extract both availableDevices and setAvailableDevices from the hook
  // Pass the environment type to optimize scanning for crowded environments
  const [availableDevices, setAvailableDevices] = useBleScanner(beaconList, setBeaconList, handlePacket, scanEnvironment);
      ...
      ListHeaderComponent={
        <>
          ...
          <View style={styles.environmentSelector}>
            <Text style={styles.sectionTitle}>BLE Scan Environment:</Text>
            <ScrollView 
              horizontal 
              showsHorizontalScrollIndicator={true} 
              contentContainerStyle={styles.environmentButtonsContainer}
            >
              {['normal', 'crowded', 'highPrecision', 'batterySaving', 'wideScan', 'emergency'].map(env => (
                <Pressable
                  key={env}
                  style={[
                    styles.environmentButton,
                    scanEnvironment === env && styles.selectedEnvironment
                  ]}
                  onPress={() => {
                    setScanEnvironment(env);
                    restartScan();
                  }}
                >
                  <Text 
                    style={[
                      ...
                      scanEnvironment === env && styles.selectedEnvironmentText
                    ]}
                  >
                    {env === 'wideScan' ? 'Wide Scan' : 
                     env === 'emergency' ? 'EMERGENCY' :
        ...
        <>
          {mode === 'navigate' && (
            <>
              ...
              {/* Add explanation of current scan environment */}
              <View style={styles.environmentInfo}>
                <Text style={styles.environmentInfoTitle}>
                  Current Scan Environment: {scanEnvironment.charAt(0).toUpperCase() + scanEnvironment.slice(1)}
                </Text>
                {scanEnvironment === 'crowded' && (
                  <Text style={styles.environmentInfoText}>
                    Optimized for busy environments with many BLE devices. Uses stronger filtering 
                    and moderate scanning rate to focus on your beacons.
                  </Text>
                )}
                {scanEnvironment === 'normal' && (
                  <Text style={styles.environmentInfoText}>
                    Standard scanning for typical environments with balanced power usage and detection rate.
                  </Text>
                )}
                {scanEnvironment === 'highPrecision' && (
                  <Text style={styles.environmentInfoText}>
                    Maximum precision scanning with higher power usage. Best when positioning 
                    accuracy is critical.
                  </Text>
                )}
                {scanEnvironment === 'batterySaving' && (
                  <Text style={styles.environmentInfoText}>
                    Reduced power consumption with slower updates and stronger signal filtering.
                  </Text>
                )}
              </View>
